...
<\body>
  <doc-data|<doc-title|Algoritmi>|<doc-author|<author-data|<author-name|>>>|<doc-author|<author-data|<author-name|Di
  Gianluca Mondini>>>>
  ...
  <\itemize>
    <item>È un metodo di ricerca in array

    <item>Non è basato su confronti

    <item>Molto efficiente
  </itemize>
  ...
  <\corollary>
    <with|font-series|bold|Legge di scansione lineare> Se non si trova
    l'elemento al suo posto, lo si cerca nelle posizioni successive fino a
    trovarlo o ad incontrare una posizione vuota.
  ...
  <\definition>
    Un agglomerato è un gruppo di elementi con indirizzi hash diversi
  </definition>

  La presenza di collisioni ed agglomerati aumenta il tempo di ricerca

  <\corollary>
    La diversa lunghezza del passo di scansione riduce gli agglomerati, ma è
    necessario controllare che la scansione visiti tutte le possibili celle
    vuote dell'array, per evitare che l'inserimento fallisca anche in
    presenza di array non pieno.
  </corollary>

  <subsubsection|Tempo medio di ricerca>

  Il tempo medio di ricerca (numero medio di confronti) dipende dal rapporto
  <math|\<alpha\>=n/k> (sempre minore di 1)
  ...
  Si può usare quando non è possibile applicare il metodo del divide et
  impera (non si sa con esattezza quali sottoproblemi risolvere e non è
  possibile partizionare l'insieme in sottoinsiemi disgiunti)

  <with|font-series|bold|Metodo:> si risolvono <with|font-series|bold|tutti>
  i sottoproblemi a partire dal basso e si conservano i risultati ottenuti
  per poterli usare successivamente (strategia
  <with|font-series|bold|bottom-up>)

  <with|font-series|bold|La complessità del metodo dipende dal numero di
  sottoproblemi>
  ...
  <\itemize>
    <item><with|font-series|bold|Sottostruttura ottima:> una soluzione ottima
    del probleme contiene la soluzione ottima dei sottoproblemi

    <item><with|font-series|bold|Sottoproblemi comuni:> un algoritmo
    ricorsivo richiederebbe di risolvere lo stesso problema più volte
  </itemize>

  <section|Algoritmi greedy (golosi)>

  <\definition>
    Negli algoritmi greedy la soluzione ottima si ottiene mediante una
    sequenza di scelte
  </definition>

  In ogni punto dell'algoritmo, viene scelta la strada che in quel momento
  sembra la migliore.

  La <with|font-series|bold|scelta locale> deve essere in accordo con la
  <with|font-series|bold|scelta globale>: scegliendo ad ogni passo
  l'alternativa che sembra la migliore non si perdono alternative che
  potrebbero rivelarsi migliori nel seguito.

  <subsection|Metodo top-down>

  Non sempre si trova la soluzione ottima ma in certi casi si può trovare uan
  soluzione approssimata (esempio del problema dello zaino)

  <subsection|Algoritmo di Huffman>

  Gli alberi sono memorizzati in uno heap (con ordinamento inverso: la radice
  è il più piccolo)

  Si fa un ciclo dove in ogni iterazione:

  <\enumerate>
    <item>Vengono estratti i due alberi con radice minore

    <item>Vengono fusi in un nuovo albero avente come etichetta della radice
    la somma delle due radici

    <item>L'albero risultante è inserito nello heap
  </enumerate>

  Il ciclo ha <math|n> iterazioni ed ogni iterazione ha complessità
  <math|O<around*|(|log<around*|(|n|)>|)>> (si eseguono 2 estrazioni ed un
  inserimento). In totale abbiamo una complessità
  ...
  <\corollary>
    Un grafo non orientato è connesso se esiste un cammino fra due nodi
    qualsiasi del grafo
  </corollary>

  <\definition>
    La <with|font-series|bold|componente connessa> è il
    <with|font-series|bold|sottografo connesso>
  </definition>

  <\definition>
    Nella componente connessa massimale nessun nodo è connesso ad un altra
    componente connessa
  </definition>

  <\definition>
    L'<with|font-series|bold|albero di copertura> è l'insieme delle
    componenti connesse massimali acicliche
  </definition>

  <\definition>
    Nel <with|font-series|bold|minimo albero di copertura> la somma dei pesi
    degli archi è minima
  </definition>
  ...
  <subsection|Alcuni problemi>

  <\description>
    <item*|Commesso viaggiatore>Date <math|n> città, è possibile partire da
    una città, attraversare ogni città esattamente una volta e tornare alla
    città di partenza, percorrendo una distanza complessiva non superiore ad
    un intero <math|k>

    <item*|N-Regine>Data una scacchiera con <math|n\<times\>n> caselle, è
    possibile posizionare su di essa <math|n> regine in modo che nessuna
    possa ``mangiare'' un altra?
  </description>

  <with|font-series|bold|La complessità è esponenziale in <math|n>>

  <\description>
    <item*|Soddisfattibilità di una formula logica>Data una formula <math|F>
    con <math|n> variabili, esiste una combinazione di valori booleani che,
    assegnati alle variabili di <math|F>, la rendono vera?
  </description>

  <with|font-series|bold|La complessità è esponenziale:
  <math|O<around*|(|2<rsup|n>|)>>>

  <subsection|Algoritmi non deterministici>

  Si aggiunge il comando <math|choice<around*|(|I|)>>, dove <math|I> è un
  insieme.
...
<\auxiliary>
  <\collection>
    <\associate|toc>
      ...
      <with|par-left|<quote|2tab>|3.3.1<space|2spc>Alberi di decisione
      <datoms|<macro|x|<repeat|<arg|x>|<with|font-series|medium|<with|font-size|1|<space|0.2fn>.<space|0.2fn>>>>>|<htab|5mm>>
      ...
      <with|par-left|<quote|2tab>|9.2.2<space|2spc>Tempo medio di ricerca
      <datoms|<macro|x|<repeat|<arg|x>|<with|font-series|medium|<with|font-size|1|<space|0.2fn>.<space|0.2fn>>>>>|<htab|5mm>>
      ...
      <vspace*|1fn><with|font-series|<quote|bold>|math-font-series|<quote|bold>|10<space|2spc>Programmazione
      dinamica> <datoms|<macro|x|<repeat|<arg|x>|<with|font-series|medium|<with|font-size|1|<space|0.2fn>.<space|0.2fn>>>>>|<htab|5mm>>
      <no-break><pageref|auto-60><vspace|0.5fn>

      <vspace*|1fn><with|font-series|<quote|bold>|math-font-series|<quote|bold>|11<space|2spc>Algoritmi
